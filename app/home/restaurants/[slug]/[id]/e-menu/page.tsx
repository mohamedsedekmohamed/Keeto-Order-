...
export default function RestaurantLinkPage() {
  ...
  // تعديل: المنيو 'menu' يظهر أولاً بشكل افتراضي
  ...
  // Scrollspy IntersectionObserver — only active when view === 'menu'
  useEffect(() => {
    ...
    >
      ...
      <div className="relative w-full h-56 overflow-hidden shrink-0 md:h-72">
        ...
        <div className={`absolute top-6 px-6 w-full flex justify-between items-center ${isRtl ? 'flex-row-reverse' : ''}`}>
          ...
          {/* زر علوي للتبديل والعودة للمنيو إذا كان في صفحة التواصل */}
          {view === 'links' && (
            <button
      ...
      <div className="flex-1 px-6 pb-10 mt-6">
        <div className="max-w-xl mx-auto">
          ...
          {view === 'menu' ? (
            // ── Menu View (المنيو يظهر هنا أولاً وبشكل افتراضي) ──
            <div className="duration-500 animate-in slide-in-from-bottom-5">
              
              {/* زر عرض روابط التواصل الاجتماعي والاتصال داخل المنيو */}
              <button
                            ...
                            {isRtl ? item.nameAr : item.name}
                              ...
                              <div className="flex items-center justify-between mt-2">
                                <span className="text-lg font-black text-[var(--primary-color)] italic">
                                  {item.price}{" "}
                                  <small className="text-[9px] not-italic">EGP</small>
                                </span>
            ...
            // ── Links View (تظهر روابط التواصل هنا عند الضغط على الزر) ──
