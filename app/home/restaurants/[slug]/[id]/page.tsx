...
export default function Home() {
  ...
  // 1. فك تشفير الـ Slug هنا أساسي لكي تقرأه الروابط والـ Context بالحروف العربية الصحيحة
  const rawSlug = params?.slug as string;
  const restaurantName = rawSlug ? decodeURIComponent(rawSlug) : "";
  
  const restaurantId = params?.id as string;
  
  // نستخدم الـ rawSlug (المشفر) في الـ basePath لتظل الروابط المتفرعة (/restaurant أو /e-menu) سليمة ولا تسبب 404
  const basePath = `/home/restaurants/${restaurantName}`;
