...
//   // --- Map slugified names to IDs in Cookies ---
//   useEffect(() => {
//     if (content?.restaurants) {
//       const slugMap = content.restaurants.reduce(
//         (acc, r) => {
//           const cleanSlug = r.name;
//           acc[cleanSlug] = r.id; // Key is now "mataam-wast-albalad" instead of "Mata'am Wast Albalad"
//           return acc;
//         },
//         {} as Record<string, string>,
//       );
//       Cookies.set("slug-map", JSON.stringify(slugMap));
//     }
//   }, [content?.restaurants]);
...
//               <Link key={cuisine.id} href={`/home/cuisines/${cuisine.id}`} className="block">
//                 <motion.div
...
//               <Link key={category.id} href={`/home/categories/${category.id}`} className="block">
//                 <motion.div
//                   whileHover={{ y: -8 }}
...
//                 // --- Updated URL string using slugify ---
//                 <Link
//                   key={restaurant.id}
//                   className="block"
...
const slugify = (text: string) => {
  ...
  return text
    ...
    .replace(/[^\u0600-\u06FF\w\-]+/g, "") // إزالة علامات التنصيص والرموز الغريبة
    .replace(/\-\-+/g, "-");
};
  ...
  useEffect(() => {
    if (content?.restaurants) {
      const slugMap = content.restaurants.reduce(
        ...
        {} as Record<string, { id: string; originalName: string }>,
      );
      Cookies.set("slug-map", JSON.stringify(slugMap), { expires: 7 });
    }
  }, [content?.restaurants]);
      ...
      <div className="relative z-10 mx-auto max-w-7xl space-y-14">
        ...
        <section className="space-y-6">
          ...
          {content?.restaurants?.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
              {content?.restaurants?.map((restaurant, index) => {
                // تحويل الاسم لـ slug مناسب للرابط
