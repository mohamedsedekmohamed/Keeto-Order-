...
const slugify = (text: string) => {
  ...
  // نقوم بعمل encodeURIComponent لضمان تشفير الحروف العربية بشكل آمن للروابط والسيرفرات
...
export default function HomePage() {
  ...
  useEffect(() => {
    if (content?.restaurants) {
      const slugMap = content.restaurants.reduce(
        (acc, r) => {
          // نستخدم الاسم الصافي (بدون encode) كمفتاح داخل الكوكيز لأن الـ Middleware يفك التشفير أولاً
          const cleanKey = r.name
            .toString()
            .toLowerCase()
            .trim()
            .replace(/\s+/g, "-")
